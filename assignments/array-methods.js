...
let allCaps = [];
runners.map((element) => {
    allCaps.push(element.first_name.toLocaleUpperCase());
});
console.log(allCaps); 
...
console.log(emails);
// Problem 2
let emailsLessThanTen = [];
emailsLessThanTen = emails.filter((element) => {
    if(element.length < 20){
        return element;
    }
});
// Problem 3
let donationsGreaterThanHundred = [];
runners.forEach((element) => {
    if(element.donation >= 100){
        donationsGreaterThanHundred.push(element)
    }
});
console.log(donationsGreaterThanHundred);